"""
单无人机避障路径规划环境 - PPO版本 (Local Perception Final)
1. 距离计算：依赖局部感知。
...
class SingleDronePPOEnv:
    ...
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        self.num_drones = 1  # 强制为单无人机
        self.rendered_env_num = min(1, self.num_envs) # 只渲染一个环境
        ...
        if show_viewer:
            self.scene = gs.Scene(
                ...
                viewer_options=gs.options.ViewerOptions(
                    ...
                    camera_pos=(3.0, -3.0, 3.0), # 调整相机视角更聚焦中心
        ...
        self.drones = []
        # 即使只用一个，保持列表结构方便兼容
        self.drone_init_positions = env_cfg.get("drone_init_positions", [[0.0, -2.5, 0.15]])
        self.drone_goal_positions = env_cfg.get("drone_goal_positions", [[0.0, 2.5, 0.15]])
        self.drone_goal_pos_tensor = torch.tensor(self.drone_goal_positions, device=self.device, dtype=gs.tc_float)
        
        drone_colors = [(0.2, 1.0, 0.2)] # 绿色
    ...
    def step(self, actions):
        ...
        # ==================== 构建观测 (Local + Top-2 Obstacles) ====================
        # 修改：移除了所有 teammate 相关的代码
        obs_list = []
